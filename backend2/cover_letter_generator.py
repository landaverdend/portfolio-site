...
You are a professional cover letter writer. You are given a position, company, cloud technologies, frameworks, and other details. You are to write a professional cover letter for the position and company mentioned.
When responding to the user, you are to respond as the human described, not as an AI.
Your responses should ALWAYS be below 600 words.
"""
...
Ensure the cover letter reflects the desired tone, highlights my technical skills (cloud technologies and frameworks), and emphasizes the personality traits that make me a strong fit for the role.
...
class CoverLetterDTO:
  ...
  cloudTechnologies: list[str]
  frameworks: list[str]

  def __repr__(self) -> str:
    return f"CoverLetterDTO(position={self.position}, company={self.company}, tone={self.tone}, wordCount={self.wordCount}, otherDetails={self.otherDetails}, cloudTechnologies={self.cloudTechnologies}, frameworks={self.frameworks})"

def generate_cover_letter(dto: CoverLetterDTO) -> str:

  prompt = COVER_LETTER_PROMPT.replace("{position}", dto.position)
  prompt = prompt.replace("{company}", dto.company)
  prompt = prompt.replace("{tone}", dto.tone)
  prompt = prompt.replace("{wordCount}", str(dto.wordCount))
  prompt = prompt.replace("{otherDetails}", dto.otherDetails)
  prompt = prompt.replace("{cloudTechnologies}", ", ".join(dto.cloudTechnologies))
  prompt = prompt.replace("{frameworks}", ", ".join(dto.frameworks))

  response = client.chat.completions.create(
    model="gpt-4",
    messages=[{"role": "system", "content": SYSTEM_COVER_LETTER_PROMPT}, {"role": "user", "content": prompt}]
  )
